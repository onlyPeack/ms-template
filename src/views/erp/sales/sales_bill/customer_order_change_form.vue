<template>
    <bill-container @reload="reload" :type="type" :id="id" ref="billContainer">
        <basic-container v-loading="loading">
                            ...
                            <template slot-scope="scope" slot="newPrice">
                                <div class="zRightClass">
                                    <el-input v-model="scope.row.newPrice"
                                              :placeholder="scope.row.source==0?'':'请输入新含税单价'"
                                              @change="publicColumnBlur(scope.row.$index,scope.row.newPrice,'newPrice', scope.row)"
                                              :disabled="type=='view'?true:scope.row.source==0?true:false"></el-input>
                                </div>
                            </template>
            ...
            <task-history v-if="formData.billNo && processId && processId!=''" :processId="processId"
                          :class="type=='view'?'processIdBTN':''"
...
<script>
    import {getBill, getHistoryBill, addBill, tempAddBill, auditBill} from "@/api/erp/sales/bill/salesOrderChange";
    import {getObj} from '@/api/erp/crm/customer/index';
    import {getSelfCompany} from "@/api/admin/company";
    import {customerOrderOption} from '@/util/table';
    import {isMobile} from "@/util/validate";
    import {getProcessId} from '@/api/erp/oa/task/taskBill';

    export default {
        name: "customer_order_change_form",
                    ...
                    column: [
                        ...
                        },
                        {
                            ...
                            click: ({value, column}) => {
                                ...
                                this.typeName = 'customerOrderType';
                ...
                if (list.length > 0) {
                    ...
                    if (this.typeName == 'customerOrderType') {
                        ...
                        getObj(list[0].mainDTO.customerId).then(res => {
                            this.formData.linkPerson = res.data.data.linkMan;
                        ...
                        this.formData.customerCode = list[0].mainDTO.customerCode;
                        this.formData.customerName = list[0].mainDTO.customerName;
                        ...
                        for (var index in list) {
                            ...
                            itemObj.source = parseInt(item.mainDTO.source) || 0;
