                        ...
                        <avue-crud :option="itemsOption" :data="formData.items"   :class="type == 'view'?'':'main-form-items'"   :summary-method="getSummaries">
                            ...
                            <template slot-scope="scope" slot="positionName">
                                <div @click="positionBtn(scope.row,scope.row.$index)" class="zLeftClass">
                                    <el-input v-model="scope.row.positionName" placeholder="请选择入库仓位" readonly
                                              :disabled="type=='view'?true:false"></el-input>
                                </div>
                            </template>
                            <template slot-scope="scope" slot="qty">
                                <div class="zRightClass">
                                    <el-input
                                            ...
                                            v-model="scope.row.qty" placeholder="请输入入库数量"
                                            :disabled="type=='view'?true:false"></el-input>
                                </div>
                            </template>
                            <template slot-scope="scope" slot="costPrice">
                                <div class="zRightClass">
                                    <el-input
                                            @blur="publicColumnBlur(scope.row.$index,scope.row.costPrice,true,'costPrice', scope.row)"
                                            v-model="scope.row.costPrice" placeholder="请输入单位成本"
                                            :disabled="type=='view'?true:false"></el-input>
                                </div>
                            </template>
                            <template slot-scope="scope" slot="costAmount">
                                <div class="zRightClass">
                                    <el-input
                                            @blur="publicColumnBlur(scope.row.$index,scope.row.costAmount,true,'costAmount', scope.row)"
                                            v-model="scope.row.costAmount" placeholder="请输入成本"
                                            :disabled="type=='view'?true:false"></el-input>
                                </div>
                            </template>
...
<script>
    import {getBill, getHistoryBill, addBill, tempAddBill,auditBill} from '@/api/erp/wms/bill/checkStockProfit';
    import {
        customerOption,
        userOption,
        sourceOption,
                ...
                if (this.id) {
                    ...
                    getSelfCompany().then(response => {
                        let formData = {
                            ...
                            inboundDate:new Date().toLocaleDateString().replace(/\//g, '-'),
                ...
                for (var i in items) {
                    if (!this.isEmpty(items[i].skuNo)) {
                        ...
                        if (this.isEmpty(items[i].qty)) {
                            this.vueMessage('warning', '第' + (i * 1 + 1) + '行的入库数量为必填项');
                            return false;
                        }
                        ...
                        if (this.isEmpty(items[i].costAmount)) {
                            this.vueMessage('warning', '第' + (i * 1 + 1) + '行的成本为必填项');
                            return false;
                        }
