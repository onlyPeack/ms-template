                ...
                <avue-form v-model="formData" :option="formOption" ref="form" @submit="handleSubmit"  :class="type == 'view'?'main-form':'main-form main-form-select'"
                    ...
                    <template slot-scope="scope" slot="items">
                        <avue-crud :option="itemsOption" :data="formData.items"    :class="type == 'view'?'':'main-form-items'" :summary-method="getSummaries">
                            ...
                            <template slot-scope="scope" slot="takingQty">
                                <div class="zRightClass">
                                    <el-input
                                            @blur="publicColumnBlur(scope.row.$index,scope.row.takingQty,true,'takingQty', scope.row)"
                                            v-model="scope.row.takingQty" placeholder="请输入盘点数量"
                                            :disabled="type=='view'?true:false"></el-input>
                                </div>
                            </template>
...
<script>
    ...
    import {customerOption, userOption, sourceOption, commodityOption, warehouseOption} from '@/util/table';
    import {getSelfCompany} from '@/api/admin/company';
    import {getProcessId} from '@/api/erp/oa/task/taskBill';
    export default {
        name: "check_stock_form",
                                ...
                                return h('div', [
                                    ...
                                    h('el-tooltip', {
                                        attrs: {
                                            class: "item",
                                            effect: "dark",
                                            content: '盘盈盘亏 = 账面数量-盘点数量 ，正数为盘亏，负数为盘盈',
        ...
        methods: {
            ...
            publicColumnBlur(index, val, type, name, item) {
                if (!isNaN(item.takingQty) ) {
                    if (type) {
                        if (name == 'takingQty') {
                            this.formData.items[index].processQty = (Number(this.formData.items[index].qty) - Number(this.formData.items[index].takingQty)).toFixed(0);
                        }
                ...
                if (list.length > 0) {
                    ...
                    if (this.typeName == 'queryType') {
                        ...
                        for (var index in list) {
                            ...
                            itemObj.takingQty = item.qty;
                            itemObj.processQty = itemObj.qty - itemObj.takingQty;
            ...
            handleCheckItemFuc() {
                ...
                for (var i in items) {
                    if (!this.isEmpty(items[i].skuNo)) {
                        ...
                        if (this.isEmpty(items[i].takingQty)) {
                            this.vueMessage('warning', '第' + (i * 1 + 1) + '行的盘点数量为必填项');
                            return false;
                        }
                    }
                }
