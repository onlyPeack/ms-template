    ...
    <bill-container @reload="reload" :type="type" :id="id" ref="billContainer">
        <basic-container v-loading="loading">
                ...
                <avue-form v-model="formData" :option="formOption" ref="form" @submit="handleSubmit"  :class="type == 'view'?'main-form':'main-form main-form-select'"
                    ...
                    <template slot-scope="scope" slot="isBatch">
                        <div class="tableItem" style="display: unset">
                            ...
                            <el-switch v-model="formData.isBatch" placeholder="请选择仓位"></el-switch>
                        </div>
                    </template>
            ...
            <associated-selector v-if="associatedVisible" ref="associatedSelector" :dialogVisible="associatedVisible"
                                 :title="associatedTitle" :isSingle="isSingle" :associatedName="associatedName" selectType="1"
...
<script>
    import {getBill, addBill, tempAddBill,auditBill} from "@/api/erp/wms/bill/purchaseStockIn";
    import {commodityOption, positionOption,warehouseOption} from '@/util/table';
    ...
    export default {
                        ...
                        [
                            ...
                            },
                            {
                                ...
                                click: ({value, column}) => {
                                    ...
                                    this.associatedTitle = '请选择关联单据';
                                    this.associatedName = 'purchaseStockIn';
                                    this.isSingle = false;
        ...
        methods: {
                ...
                if (list.length > 0) {
                    ...
                    if (this.associatedName == 'purchaseStockIn') {
                        ...
                        for (var index in list) {
                            ...
                            itemObj.qty = Number(item.qty - item.stockInPurchaseQty + item.stockReturnQty);
                            itemObj.maxQty = Number(item.qty - item.stockInPurchaseQty + item.stockReturnQty);
                            itemObj.notStockInQty = itemObj.maxQty;
                            ...
                            itemObj.tax = (Number(itemObj.amount) * (Number(item.taxRate) / 100)).toFixed(this.sysParam.amountDecimal);
            ...
            qtyBlur(index, val) {
                if (!isNaN(val)) {
                    if (this.formData.items[index].maxQty && val > this.formData.items[index].maxQty) {
                        this.vueMessage('warning', '你输入的入库数量不能大于可入库数量!');
            ...
            handleSubmitPublic(list) {
                if (list.length > 0) {
                    ...
                    if (this.typeName == 'positionType') {
                        if(this.formData.isBatch&&this.positionIndex===0){
                            for (let i in this.formData.items) {
                                this.formData.items[i].positionName = list[0].name;
                                this.formData.items[i].positionId = list[0].id;
                            }
                        }else{
            ...
            //新增
            handleAdd() {
                this.handleBillAdd(this,{path: '/purchase_stock_in_form', name: '采购入库制单'});
            },
