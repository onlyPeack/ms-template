<template>
  <div>
    ...
    <el-table :key='tableKey' :data="list" border fit highlight-current-row style="width: 100%;margin-top: 10px;" @row-click="handlerRowClick">
    ...
    <el-dialog v-if="positionManager_btn_user" :visible.sync="dialogUserVisible"  append-to-body>
      <position-user :positionId="positionId" @closeUserDialog="closeUserDialog" ref="positionUser" ></position-user>
    </el-dialog>
    <el-dialog v-if="positionManager_btn_group" :visible.sync="dialogGroupVisible"  append-to-body>
      <position-group :positionId="positionId" @closeGroupDialog="closeGroupDialog" ref="positionGroup" ></position-group>
    </el-dialog>
     <el-dialog v-if="positionManager_btn_depart" :visible.sync="dialogDepartVisible"  append-to-body>
...
export default {
  ...
  created() {
    ...
    getTypeValue('org_position').then(response => {
      this.positionOptions = response.data.data.rows;
    });
  },
  computed: {
    ...mapGetters([
