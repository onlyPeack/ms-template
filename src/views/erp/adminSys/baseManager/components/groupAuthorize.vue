  ...
  <el-row>
    ...
    <el-col :span="9" style='margin-top:15px;margin-right:10px'>
      ...
      <el-tree class="filter-tree" :render-content="renderContent" check-strictly :data="treeData" show-checkbox node-key="id" :expand-on-click-node="false" highlight-current
        :props="defaultProps" :filter-node-method="filterNode" ref="menuTree" @node-click="getNodeData" :default-expand-all="false" style="height: 50vh;overflow: auto;">
      </el-tree>
    </el-col>
...
export default {
  ...
  methods: {
    ...
    handleCheckChange(data, check, currentNode){
      if(check){
      ...
      pageAuthorize(this.listQuery).then(response => {
        ...
        getElementAuthorize(this.groupId).then(data => {
          ...
          for (let i = 0; i < data.data.data.length; i++) {
            const id = data.data.data[i]
            if (obj[id] !== undefined && toggle[id] === undefined) {
              this.$refs.elementTable.toggleRowSelection(obj[data.data.data[i]]);
              toggle[id] = true;
            }
          }
        });
    ...
    initAuthoritys() {
      getMenuAuthorize(this.groupId).then(data => {
        const result = [];
        for (let i = 0; i < data.data.data.length; i++) {
          result.push(data.data.data[i].id);
        }
