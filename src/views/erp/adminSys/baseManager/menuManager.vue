<template>
  <div class="app-container calendar-list-container" style="padding: 10px 10px;">
    ...
    <el-row>
      ...
      <el-col :span="16" style='margin-top:15px;'>
        <el-card class="box-card">
          <el-form :label-position="labelPosition" label-width="80px" :model="form"  :rules="rules" ref="form">
            ...
            <el-form-item label="上级菜单" prop="parentIds">
              ...
              <el-cascader expand-trigger="hover" :options="treeData" v-model="form.parentIds"
                           @change="handleCascaderChange" :change-on-select="true" filterable
                           :props="cascaderProps" style="width: 300px;" :disabled="formEdit"></el-cascader>
            ...
            <el-form-item label="可见" prop="visible">
              <el-radio v-model="form.visible" label="1">显示</el-radio>
              <el-radio v-model="form.visible" label="0">隐藏</el-radio>
            </el-form-item>
        ...
        <el-card class="box-card" style='margin-top:15px;'>
          <span >按钮或资源</span>
          <menu-element :menuId='currentId' ref="menuElement" style="margin-top: 10px;"></menu-element>
        </el-card>
      </el-col>
    </el-row>
  </div>
  ...
  export default {
    ...
    created() {
      ...
      getTypeValue('authority_menu').then(response => {
        this.typeOptions = response.data.data.rows;
      });
    },
    computed: {
      ...mapGetters([
    ...
    methods: {
      getList() {
        fetchTree(this.listQuery).then(data => {
          this.treeData = this.getTreeData(data.data);
        });
      },
      getTreeData(data) {
      ...
      handleOpenTree() {
        ...
        for (var i in this.$refs.menuTree.store.nodesMap) {
          this.$refs.menuTree.store.nodesMap[i].expanded = this.isOpenTree;
        }
      },
      ...
      create() {
        if (!this.form.code || this.form.code == '') {
          this.$message.error('路径编码为必填项，请填写了再提交');
          return;
        }
