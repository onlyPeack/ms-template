...
<script>
    ...
    import {addGoodsTag,updateGoodsTag,page,updateStatus,deleteTag} from '@/api/erp/saas/goodsTag'
    import {allBrand} from '@/api/erp/goods/brand';
                ...
                this.$confirm('此操作将永久删除该记录，是否继续?', '提示', {
                    ...
                    deleteTag(row.id).then(response => {
                        if (Number(response.data.code) === 2000||Number(response.data.code)===200) {
                            this.showSuccess('删除商品标签成功')
                        }else {
                            this.showError('删除商品标签失败,'+response.data.msg||response.data.data)
                        }
                    },error => this.showError('删除商品标签失败,'+error));
                });
            },
