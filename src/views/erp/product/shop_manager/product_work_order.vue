...
<script>
    import {selectPage, selectPageByItem, batchDelBill,updateStatus} from "@/api/erp/product/shop_manager/product_work_order";

    export default {
        name: "product_work_order",
        components: {
        ...
        methods: {
            ...
            cancelBtn(val){
                this.$confirm('你确定执行该操作, 是否继续?', '提示', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning'
                }).then(() => {
                    updateStatus(val).then((res) => {
            ...
            handleAdd() {
                this.handleBillAdd(this,{path: '/product_work_order_form', name: '报工单制单'});
            },
            handleSwitchChange() {
                this.isShow = false;
                this.$nextTick(() => {
            ...
            handleCancels(){
                ...
                let flag = true;
                if (flag) {
                    this.$confirm('你确定执行该操作?', '提示', {
                        confirmButtonText: '确定',
                        cancelButtonText: '取消',
                        type: 'warning'
                    }).then(() => {
                        updateStatus(this.selectedRows).then((res) => {
                            if (res.data.code == 200 || res.data.code == '2000') {
                                this.$message({
                                    type: "success",
                                    message: "操作成功!"
